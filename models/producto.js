...
getProductCat = async(id_categoria) => {
    try {
        const query =
            "SELECT id, nombre, descripcion ,precio , imagen FROM ??  WHERE id_categoria = ?";
        const params = [process.env.TABLA_PRODUCTO, id_categoria];
        const rows = await pool.query(query, params);
        return rows[0];
    } catch (error) {
        console.log(error);
